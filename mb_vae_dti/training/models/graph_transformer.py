"""
Graph transformer module for the discrete diffusion model of DTITree.

Graph transformer module (and its layers) for the reverse diffusion process.
It predicts the node & edge type probabilities \hat{p}^G (\hat{p}^X, \hat{p}^E) 
of the (next) clean(er) graph from the noise discretized & augmented graph G^t.
"""
