...
class DTIDataset(h5torch.Dataset):
    ...
    def _preload_unstructured_y_data(self):
        ...
        logger.info(f"Pre-loaded unstructured Y data for {len(self.unstructured_y_data)} columns: {list(self.unstructured_y_data.keys())}")
    ...
    def __getitem__(self, idx: int) -> Dict[str, Any]:
        ...
        for name, path in self.drug_feature_paths.items():
            if path in item_flat:
                structured_item['drug']['features'][name] = item_flat[path]
        ...
        for name, path in self.target_feature_paths.items():
            if path in item_flat:
                structured_item['target']['features'][name] = item_flat[path]
